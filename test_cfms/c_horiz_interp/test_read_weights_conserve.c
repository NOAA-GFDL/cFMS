...
int main() {
    ...
    int interp_id = cFMS_horiz_interp_read_weights_conserve(weight_filename, weight_file_src,
        &nlon_src, &nlat_src, &xsize, &ysize, &isc, &iec, &jsc, &jec, &src_tile);

    // remap
    bool convert_cf_order = false;
    cFMS_horiz_interp_base_2d_cdouble(&interp_id, data_src, data_dst,
        NULL, NULL, NULL, NULL, NULL, NULL, &convert_cf_order);
